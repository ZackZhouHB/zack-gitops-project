...
async def startup():
    global weaviate_client, rag_service, rag_agent, doc_worker
    logger.info("Starting RAG v1 API...")
    ...
    rag_service = RAGService(weaviate_client, settings)
...
@app.get("/")
async def root():
    return {"status": "healthy", "service": "RAG v1"}
