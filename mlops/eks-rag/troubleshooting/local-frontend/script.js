...
async function uploadFile() {
    ...
    try {
        ...
        fileInput.value = '';
        setTimeout(() => { statusDiv.innerHTML = originalContent; }, 5000);
    }
}
...
async function processDocuments() {
    const statusDiv = document.getElementById('document-action-status');
    const originalContent = statusDiv.innerHTML;
    statusDiv.innerHTML = `<p style="color: #007bff;"><i>Processing documents...</i></p>`;
...
async function processDocuments() {
    ...
    showLoading('Process documents into vector databse...');
    try {
        const response = await fetch(`${API_BASE_URL}/documents/auto-index`, {
            method: 'POST'
        });
        if (!response.ok) {
            throw new Error(`Processing trigger failed: ${response.status}`);
        }
        const result = await response.json();
        hideLoading();
