"""Incident tool — triage infrastructure incidents using KB search + pattern matching."""
...
async def assess_incident(
    ...
    search_result = await kb_search(query=search_query, top_k=3)

    related_docs = [r.source_uri for r in search_result.results] if search_result.success else []
