...
variable "users" {
 description = "List of users with their principal IDs and permission set ARNs"
 type    = list(object({
  principal_id   = string
...
- A "terraform.tfvars" file to define and manage assignments using variables, here to define each unique assignment per request with each "principal_id" and "permission_set_arn" to achieve dynamically assigning users to different accounts with various permission sets.
